...
fvm flutter test  # Tests MUST FAIL (red phase)
...
fvm flutter test  # Tests MUST PASS (green phase)
...
6. **Build & Tests**: `fvm flutter analyze` and `fvm flutter test` must pass
...
7. **Verify**: Run `fvm flutter test` to ensure all tests pass
8. **Build**: Run `fvm flutter analyze` to ensure quality standards met
...
fspec add-virtual-hook AUTH-001 post-implementing "fvm flutter analyze" --blocking
...
**Example**:
```bash
# Work unit AUTH-001 has virtual hook: "fvm flutter analyze"
...
# Execution order:
# 1. AUTH-001 virtual hook: fvm flutter analyze
...
fspec add-virtual-hook AUTH-001 pre-validating "fvm flutter analyze" --blocking
...
**Quality gates**:
```bash
fspec add-virtual-hook AUTH-001 post-implementing "fvm flutter test" --blocking
fspec add-virtual-hook AUTH-001 pre-validating "eslint" --git-context --blocking
```
...
Next steps:
  ...
  3. Run tests to validate: fvm flutter test
...
4. AI runs tests: `fvm flutter test`
